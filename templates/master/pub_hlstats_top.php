         ...
         <? echo Model('PublicData')->menu($x, $acp_system['acp_strona_www'], $acp_system['acp_nazwa']); ?>
           ...
           <div class="col-lg-12">
            <div class="box box">
                ...
                <? $id = Get::int('id');
                if(!empty($id)): ?>
                <? $dane_pobrania = Model('PublicData')->hlstats_top_details_dane($id); tytul_strony("Top50 - $dane_pobrania->mod - $dane_pobrania->data"); ?>
                <div class="col-md-12">
                ...
                <div class="col-md-12">
                  <table id="hlstats_details" class="table table-bordered table-striped" width="100%">
                   ...
                   <tbody>
                     <? $dane = Model('PublicData')->hlstats_top_details($id);
                     $i = 1;
                     foreach ($dane as $row):
                       $row->steam_comunity = $Steam->toCommunityID("STEAM_0:".$row->steam);
                     ?>
                     <tr>
                       <td scope="row"><?= $i++ ?></td>
                       ...
                       <td><?= $row->skill ?></td>
                       <td><?= Date::secund($row->connection_time) ?></td>
                       <td><?= $row->kills ?></td>
                       <td><?= $row->deaths ?></td>
                       <td><?= $row->kpd ?></td>
                       <td><?= $row->headshots ?></td>
              ...
              <table id="hlstats_top" class="table table-bordered table-striped" width="100%">
                ...
                <tbody>
                  <?
                  $dane = Model('PublicData')->hlstats_top_list($xx);
                  foreach ($dane as $row):
                  ?>
                  <tr>
                    <td><?= $row->id ?></td>
...
<?= Model('DataTable')->table([
  ['name' => '#serwery'],
  ['name' => '#hlstats_top', 'sort' => '2', 'sort_type' => 'desc'],
  ['name' => '#hlstats_details', 'display' => '25'],
]); ?>
