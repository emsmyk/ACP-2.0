  ...
  <div class="content-wrapper">
    <div class="container">
      <section class="content">
    		<div class="row">
    		  <div class="col-lg-8">
            <? $LastWpis = Controller('Default')->index()['LastWpis']; ?>
            <div class="box box-widget">
              <div class="box-header with-border">
          		  <div class="user-block">
          			<img class="img-circle" src="<?= User::Avatar($LastWpis->steam_avatar); ?>">
          			<span class="username"><?= User::printName($LastWpis->u_id, true); ?> - <span style="word-wrap:break-word;"><?= $LastWpis->tytul; ?></span></span>
          			<span class="description"><a href="?x=wpisy&xx=category&nazwa=<?= $LastWpis->kategoria_nazwa; ?>&id=<?= $LastWpis->kategoria ?>"><?= $LastWpis->kategoria_nazwa; ?></a> - <?= Date::relative($LastWpis->data); ?></span>
          		  </div>
          		</div>
              <div class="box-body ogloszonko">
          		  <p style="word-wrap:break-word;"><?= $LastWpis->text; ?></p>
          		</div>
              <div class="box-footer">
                <a href="?x=wpisy&xx=wpis&wpis=<?= Text::clean($LastWpis->tytul); ?>&wpisid=<?= $LastWpis->id; ?>"><button type="button" class="btn btn-default btn-xs"><i class="fa fa-share"></i> Czytaj całość</button></a>
          		  <span class="pull-right text-muted"><?= Controller('Default')->index()['LastWpisComents'] ?> komentarzy - <?= Controller('Default')->index()['LastWpisComentsUsers'] ?> komentujacych</span>
              </div>
            </div>
    		  </div>
          <div class="col-md-4">
             <div class="box box-widget">
               ...
               <div class="box-body">
                 <? foreach(Controller('Default')->index()['FiveLastWpis'] as $wpis) { ?>
                 <p><a href="?x=wpisy&=wpis&wpis=<?= Text::clean($wpis->tytul); ?>&wpisid=<?= $wpis->id; ?>"><?= $wpis->tytul ?></a> <?= $wpis->data ?></p>
                <? } ?>
               </div>
             </div>
          </div>
          ...
          <div class="col-md-4">
            <div class="box box-solid">
              ...
              foreach ($dane as $index => $row):
                ...
                $row->banner = Model('PublicData')->serwer_banner($row->serwer_id);
                ...
                <div class="row">
                  <div class="col-sm-3 col-xs-6">
                    <div class="description-block border-right">
                      <h5 class="description-header"><?= SQL::one("SELECT COUNT(`ip`) as `count` FROM `acp_serwery` WHERE `test_serwer` = '0'"); ?></h5>
                      <span class="description-text">Serwerow</span>
                    </div>
                  </div>
                  <div class="col-sm-3 col-xs-6">
                    <div class="description-block border-right">
                      <h5 class="description-header"><?= SQL::one("SELECT sum(`graczy`) AS `graczy` FROM `acp_serwery` WHERE `test_serwer` = '0'"); ?></h5>
                      <span class="description-text">Graczy</span>
                    </div>
                  </div>
                  <div class="col-sm-3 col-xs-6">
                    <div class="description-block border-right">
                      <h5 class="description-header"><?= SQL::one("SELECT sum(`max_graczy`) AS `max_graczy` FROM `acp_serwery` WHERE `test_serwer` = '0'"); ?></h5>
                      <span class="description-text">Slotow</span>
                    </div>
                  </div>
                  <div class="col-sm-3 col-xs-6">
                    <div class="description-block">
                      <h5 class="description-header"><? $procentowe_zapelnienie = SQL::one("SELECT sum(`graczy`)*100/sum(`max_graczy`) AS `graczy` FROM `acp_serwery` WHERE `test_serwer` = '0'"); echo round($procentowe_zapelnienie); ?> %</h5>
...
$(function () {
  ...
  var salesChartData = {
    labels  : [
      <?
      $dane_wykres_q = SQL::all("SELECT LEFT(`data`, 16) AS `data_cr` FROM `acp_serwery_logs_hour` GROUP BY `data_cr` ORDER BY `data_cr` DESC LIMIT 24;");
      foreach($dane_wykres_q as $dane_wykres) {
      print("'");  print(Date::relative($dane_wykres->data_cr)); print("',");
      }
      ?>
    ],
    datasets: [
      {
        ...
        data                : [
           <?
          $dane_wykres_q = SQL::all("SELECT LEFT(`data`, 16) AS `data_cr`, SUM(`sloty`) AS sum_sloty FROM `acp_serwery_logs_hour` GROUP BY `data_cr` ORDER BY `data_cr` DESC LIMIT 24;");
          foreach($dane_wykres_q as $dane_wykres) {
      ...
      },
      {
        ...
        data                : [
           <?
          $dane_wykres_q = SQL::all("SELECT LEFT(`data`, 16) AS `data_cr`, SUM(`graczy`) AS sum_graczy FROM `acp_serwery_logs_hour` GROUP BY `data_cr` ORDER BY `data_cr` DESC LIMIT 24; ");
          foreach($dane_wykres_q as $dane_wykres) {
