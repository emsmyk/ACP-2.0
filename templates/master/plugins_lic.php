<?php
    ...
    $serwer = SQL::one("SELECT `serwer_id` FROM `acp_serwery` WHERE `ip` = '$dane_serwera_ip[0]' AND `port` = '$dane_serwera_ip[1]' LIMIT 1");
    ...
    $plugin_hash =  SQL::one("SELECT `lic_hash` FROM `acp_pluginy` WHERE `lic_name` = '$pluginID' LIMIT 1");
