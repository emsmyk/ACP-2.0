<?
$KonkurencjaController = Controller('Konkurencja');
?>
<style>
...
//
$fora_xml_q = SQL::all("SELECT `nazwa`, `url`, `ilosc`, `code`, `dane_time`  FROM `acp_konkurencja`;");
foreach ($fora_xml_q as $fora_xml) {
  $KonkurencjaController->rss($fora_xml->url, $fora_xml->code, $fora_xml->dane_time, $fora_xml->ilosc);
}
...
$fora_xml_q = SQL::all("SELECT `nazwa`, `color`, `url`, `code` FROM `acp_konkurencja`;");
foreach ($fora_xml_q as $fora_xml):
  $fora_xml->file_xml = file_get_contents("www/rss/$fora_xml->code.htm");
      ...
      <div class="col-lg-12">
        <div class="box">
          ...
          <div class="box-body">
            ...
            ?>
              <li class="list-group-item bg-<?= $strony_dane_kolor[$item->code] ?>"><?= $strony_dane[$item->code] ?> <span class="badge badge-primary badge-pill"><?= Date::relative($item->pubDate); ?></span></li>
      ...
      <div class="col-lg-12">
        <div class="box">
          ...
          <div class="box-body">
            <ul class="list-group">
              <?
              $lista_stron_q = SQL::all("SELECT `id`, `nazwa` , `code` FROM `acp_konkurencja`");
