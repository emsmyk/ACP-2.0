...
?>
  <div class="row">
    <? if(!empty(Controller('Pluginy')->id)):
      $dane = Controller('Pluginy')->edit();
    ?>
    <div class="col-lg-8">
      <div class="box box">
      ...
      <div class="box box">
        <div class="box-header">
          <h3 class="box-title"><i class="fa fa-file"></i>  Pliki (<?= $dane->ilosc_plikow; ?>)</h3>
        </div>
        <div class="box-body">
          <div class="col-lg-12">
    ...
    <div class="col-xs-12">
            ...
            <thead>
                ...
                <th>ID</th>
						...
						<tbody>
            <?
            foreach (Controller('Pluginy')->index() as $pluginy) {
            ?>
              <tr class="odd gradeX">
                <td><?= $pluginy->id ?></td>
                <td><a href="<?= "?x=$x&id=$pluginy->id" ?>"><?= $pluginy->nazwa ?></a></td>
                <td><?= $pluginy->login ?></td>
    ...
    <div class="modal fade" id="edytuj_pliki_<?= $pliki->id ?>">
      <div class="modal-dialog modal-xl">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <form name='edytuj_plik' method='post'>
  ...
  <div class="modal fade" id="pliki">
    <div class="modal-dialog modal-xl">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST" ENCTYPE="multipart/form-data" name="wgraj_plik">
            <input type='hidden' name='id' value='<?= $dane->id ?>'>
        ...
        <div class="modal-body">
          <form action="<?= "?x=$x&id=$dane->id"; ?>" method='post' name="wgrywarka">
            ...
            foreach ($servers->servers as $serwery):
              $serwery->istnieje = (in_array($serwery->serwer_id, $plugin_serwer->lista) && $serwery->serwer_on == 1 && $serwery->cronjobs == 1) ? '<small class="label label-warning"><i class="fa fa-clock-o"></i> Plugin Wgrany</small>' : '' ;
            ?>
             <div class="checkbox">
               <label>
