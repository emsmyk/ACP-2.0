		...
		<div class="col-xs-12">
          ...
          <ul class="list-group">
            ...
            foreach ($servers->servers as $key => $value):
              $value->liczba_raportow = SQL::one("SELECT COUNT(*) FROM `raport_serwer` WHERE `serwer_id` = ".$value->serwer_id." ");
            ?>
  ...
  <div class="row">
		<div class="col-xs-12">
			<div class="box box">
				...
				<div class="box-body">
          <?
          $raporty= SQL::all("SELECT * FROM `raport_serwer` WHERE `serwer_id` = $serwer ORDER BY `rok`, `miesiac` DESC");
          foreach ($raporty as $key => $value):
            $raport_admini = SQL::all("SELECT * FROM `raport_opiekun` WHERE `serwer` = $value->serwer_id AND `miesiac` = $value->miesiac AND `rok` = $value->rok");
            $value->admin_miesiaca = $Steam->toCommunityID($value->admin_miesiaca);
          ?>
          <div class="invoice">
            <div class="row">
              <div class="col-xs-12">
                ...
                <table class="table table-striped">
                  ...
                  <tbody>
                    <? foreach ($raport_admini  as $value2):
                      ...
                      $value2->steamid = $Steam->toCommunityID($value2->steamid);
                    ...
                    <tr>
                      ...
                      <td><?= $value2->grupa ?></td>
                      <td><?= Date::secund((int)$value2->serwer_czaspolaczenia) ?></td>
