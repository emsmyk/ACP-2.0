		...
		<div class="col-xs-12">
          ...
          <ul class="list-group">
            <? foreach ($months as $key => $value):
              $liczba_raportow = SQL::one("SELECT COUNT(*) FROM `raport_serwer` WHERE `miesiac` = ".$months_number[$key]." AND `rok` = ".$rok_number[$key]."");
            ?>
  ...
  <div class="row">
		<div class="col-xs-12">
			<div class="box box">
				...
				<div class="box-body">
          <? $raport = SQL::all("SELECT * FROM `raport_serwer` WHERE `miesiac` = $miesiac AND `rok` = $rok");
          foreach ($raport as $key => $value):
            $raport_admini = SQL::all("SELECT * FROM `raport_opiekun` WHERE `serwer` = $value->serwer_id AND `miesiac` = $miesiac AND `rok` = $rok");
