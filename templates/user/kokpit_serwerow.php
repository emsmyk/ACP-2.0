  ...
  <div class="row">
	...
	<div class="row">
		<div class="col-lg-4">
          ...
          <?
          $dostep = SQL::all("SELECT `serwer_id`, `cronjobs`, `mod`, `ftp_user`, `ftp_haslo`, `ftp_host`, `rcon` FROM `acp_serwery` ORDER BY `acp_serwery`.`istotnosc` ASC");
        ...
        <div class="box-body table-responsive no-padding">
          ...
          foreach ($dostep as $value):
            if($value->cronjobs == 1):
              $value->prace = SQL::row("SELECT `reklamy`, `bazy`, `mapy`, `hextags`, `help_menu`, `uslugi` FROM `acp_serwery_cronjobs` WHERE `serwer` = $value->serwer_id LIMIT 1");
