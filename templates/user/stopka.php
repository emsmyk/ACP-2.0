...
if(isset($_POST['user_ustawienia'])) {
	...
	SQL::query("UPDATE `acp_users` SET `szablon` = '$from->szablon', `uklad_16_4` = '$from->uklad_16_4', `pudelkowy` = '$from->pudelkowy', `menu` = '$from->menu', `prawy_kolor` = '$from->prawy_kolor' WHERE `user` = $player->user;");
					 ...
					 <p>
						  <select class="form-control select2" name="id_grupy_sessja" style="width: 100%;">
						   <option selected="selected" value="">Domyślna</option>
							 <?
							 $acp_grupa_sessja_lista = SQL::all("SELECT `id`, `nazwa` FROM `acp_users_grupy`");
