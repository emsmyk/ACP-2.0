...
switch($typ){
	//kosz
	case 0:
		$liczba_wiadomosci = SQL::one("SELECT COUNT(*) FROM `acp_messages` WHERE m_type = $typ AND `m_czyja` = $player->user ORDER BY m_status ASC, m_id DESC;");
		$wiadomosci_q = SQL::all("SELECT * FROM `acp_messages` WHERE m_type = $typ AND `m_czyja` = $player->user ORDER BY m_status ASC, m_id DESC LIMIT $limit1, $limit2;");
	break;
	//odebrane
	case 1:
		$liczba_wiadomosci = SQL::one("SELECT COUNT(*) FROM `acp_messages` WHERE m_to = $player->user AND m_type = $typ ORDER BY m_status ASC, m_id DESC;");
		$wiadomosci_q = SQL::all("SELECT *, (SELECT `login` FROM `acp_users` WHERE `user` = `m_from`) AS login FROM `acp_messages` WHERE m_to = $player->user AND m_type = $typ ORDER BY m_status ASC, m_id DESC LIMIT $limit1, $limit2;");
	break;
	//wyslane
	case 2:
		$liczba_wiadomosci = SQL::one("SELECT COUNT(*) FROM `acp_messages` WHERE m_from = $player->user AND m_type = $typ ORDER BY m_status ASC, m_id DESC;");
		$wiadomosci_q = SQL::all("SELECT *, (SELECT `login` FROM `acp_users` WHERE `user` = `m_to`) AS login FROM `acp_messages` WHERE m_from = $player->user AND m_type = $typ ORDER BY m_status ASC, m_id DESC LIMIT $limit1, $limit2;");
	break;
	//robocze
	case 3:
		$liczba_wiadomosci = SQL::one("SELECT COUNT(*) FROM `acp_messages` WHERE `m_type` = $typ AND `m_czyja` = $player->user ORDER BY `m_status` ASC, `m_id` DESC;");
		$wiadomosci_q = SQL::all("SELECT *, (SELECT `login` FROM `acp_users` WHERE `user` = `m_to`) AS login FROM `acp_messages` WHERE `m_type` = $typ AND `m_czyja` = $player->user ORDER BY `m_status` ASC, `m_id` DESC LIMIT $limit1, $limit2;");
	break;
}
?>
  <div class="row">
              ...
              <a href="?x=wiadomosci&xx=skrzynka&type=<?= $typ ?>"><button type="button" class="btn btn-default btn-sm"><i class="fa fa-refresh"></i></button></a>
                ...
                ?>
                  <tr>
                   ...
                   <td class="mailbox-date"><?= Date::relative($wiadomosci->m_date); ?></td>
                  </tr>
                <? }
                else:?>
                  <tr>
