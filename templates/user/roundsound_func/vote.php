		...
		<div class="col-xs-12">
            ...
            <thead>
                ...
                <th>ID</th>
						...
						<tbody>
            <?
            $lista = SQL::all("SELECT *, (SELECT `nazwa` FROM `rs_roundsound` WHERE `id` = `roundsound` LIMIT 1) AS `roundsound_nazwa`, (SELECT `nazwa` FROM `rs_utwory` WHERE `id` = `utwor` LIMIT 1) AS `utwor_nazwa`  FROM `rs_vote` ORDER BY `data` DESC");
