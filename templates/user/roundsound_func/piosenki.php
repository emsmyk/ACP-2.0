		...
		<div class="col-xs-12">
            ...
            <thead>
                ...
                <th>ID</th>
						...
						<tbody>
            <?
            $lista = SQL::all("SELECT *, (SELECT `nazwa` FROM `rs_roundsound` WHERE `id` = `roundsound_propozycja` ) AS `roundsound_nazwa`, (SELECT `login` FROM `acp_users` WHERE `user` = `akcept` LIMIT 1) AS `login_akceptujacego` FROM `rs_utwory` ORDER BY `id` DESC");
