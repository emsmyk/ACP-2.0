...
$wgrywarka_wykonanych = SQL::one("SELECT COUNT(`id`) FROM `acp_wgrywarka` WHERE `status` = '1' ");

?>
  <div class="row">
    <? if(!empty($id)):
      $dane = SQL::row("SELECT *, `serwer_id` AS `id_serwera`,(SELECT `login` FROM `acp_users` WHERE `user` = `u_id` LIMIT 1) AS `login`, (SELECT `nazwa` FROM `acp_serwery` WHERE `serwer_id` = `id_serwera` LIMIT 1) AS `nazwa_serwera` FROM `acp_wgrywarka` WHERE `id` = $id; ");
  ...
  <div class="row">
    <div class="col-lg-8">
            ...
            <thead>
                ...
                <th>Serwer</th>
						...
						<tbody>
            <?
            $wgrywarka_q = SQL::all("SELECT *, `serwer_id` AS `id_serwera`,(SELECT `login` FROM `acp_users` WHERE `user` = `u_id` LIMIT 1) AS `login`, (SELECT `mod` FROM `acp_serwery` WHERE `serwer_id` = `id_serwera` LIMIT 1) AS `mod_serwera` FROM `acp_wgrywarka` ORDER BY `id` DESC");
    ...
    <div class="col-lg-4 col-xs-12">
      <div class="box box">
        ...
        <div class="box-body">
          <?
          $bledy_all = SQL::all("SELECT `serwer_id` AS `id_srv`, `tekst`, `tekst_admin`, `data`, (SELECT `mod` FROM `acp_serwery` WHERE `serwer_id` = `id_srv`) AS `mod` FROM `acp_serwery_bledy` WHERE `status` = '1' ORDER BY `data` DESC LIMIT 10");
          if(!empty($bledy_all)):
            foreach ($bledy_all as $row):?>
              <p><b>[<?= $row->mod ?>]</b> <?= $row->tekst ?> - <?= Date::relative($row->data)?></p>
            <? endforeach; ?>
          <? else: ?>
...
<?= Model('DataTable')->table([
  ['name' => '#example', 'sort_type' => 'desc', 'display' => '20']
]); ?>
</body>
</html>
