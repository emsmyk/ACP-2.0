...
if(Permission::check($dostep->serwery_det_RaportOpiekuna, false) == 1):
  ...
  $raport->przycisk = '';
  $raport->save = SQL::row("SELECT * FROM `raport_serwer` WHERE `miesiac` = '".$raport->data['ubiegly_miesiac']."' AND `rok` = '".$raport->data['rok']."' AND `serwer_id` = '$serwer_id' LIMIT 1");
  if(empty($raport->save)){
    $raport->get->hls_graczy = Model('Hlstats')->ilosc_graczy($srv_dane->prefix_hls);
    $raport->get->sb = Model('Hlstats')->raport_opiekuna($srv_dane->prefix_sb);
    $raport->get->old_data = SQL::row("SELECT `finanse_koszt`, `gt_rank`, `gt_low`, `gt_hight` FROM `raport_serwer` WHERE `serwer_id` = '$serwer_id' AND `miesiac` = '".$raport->data['ubiegly_miesiac']."' LIMIT 1");
  }
?>
