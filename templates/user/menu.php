...
$menu_q = SQL::all("SELECT `id`, `nazwa`, `ikona`, `nazwa_wys`, `wlaczony`, `menu`, `menu_kategoria` FROM `acp_moduly` WHERE `menu` != 0; ");
?>
    ...
    <li class="header">Podstawowe Funkcje</li>
			...
			if(in_Array($menu->nazwa, $moduly)) {
				if($menu->menu_kategoria == 0) {
					if($menu->menu == 1) {
						...
						";
						$sub_menu_q = SQL::all("SELECT * FROM `acp_moduly_menu` WHERE `modul_id` = $menu->id; ");
		...
		<li class="treeview <? akt_kat("serwery_det");?>">
			...
			<ul class="treeview-menu">
				<?
				if($player->role == 1) {
					$acp_dostep_serwer = SQL::all("SELECT `serwer_id`,`istotnosc`, `mod` FROM `acp_serwery` ORDER BY `istotnosc` ASC");
					...
					$acp_dostep_serwer = SQL::all("SELECT `serwer_id`,`istotnosc`, `mod` FROM `acp_serwery` WHERE `ser_a_jr` = '$player->user' OR `ser_a_opiekun` = '$player->user' OR `ser_a_copiekun` = '$player->user' ORDER BY `istotnosc` ASC");
					foreach($acp_dostep_serwer as $acp_dostep_s){ ?>
						<li class="<? akt_li_srv("serwery_det", $acp_dostep_s->serwer_id);?>"><a href="?x=serwery_det&serwer_id=<?= $acp_dostep_s->serwer_id ?>"><i class="fa fa-circle-o"></i> <?= $acp_dostep_s->istotnosc ?> - <?= $acp_dostep_s->mod ?></a></li>
					<? }
				}?>
