...
if(isset($co) && isset($xx)){
    ...
    if(isset($_POST['dodaj_from'])){
      ...
      SQL::insert('acp_slots_serwery',[
          'serwer' => $xx,
          'min' => $form->min,
          'max' => $form->max,
          'h_start' => $form->h_start,
          'h_koniec' => $form->h_koniec,
          'style' => $form->style,
        ]
      );
	...
	<div class="row">
      ...
      <div class="box box">
          ...
          <table class="table">
            ...
            <tbody>
              <?
              $variable = SQL::all("SELECT `serwer_id`, `nazwa`, `mod`, `max_graczy`, `min`, `max`, `h_start`, `h_koniec`, `style` FROM `acp_serwery` LEFT JOIN `acp_slots_serwery` ON `acp_serwery`.`serwer_id` = `acp_slots_serwery`.`serwer`;");
              ...
              <?
                  ...
                  $value->ilosc_graczy = SQL::one("SELECT SUM(`graczy`) FROM `acp_serwery_logs_hour` WHERE `serwer_id` = $value->serwer_id AND `data` BETWEEN '$data1' AND '$data2'");
                  $value->sr_ilosc_graczy = round($value->ilosc_graczy / ($value->h_koniec - $value->h_start + 1), 2);
                  $value->sr_zapelnienie = ($value->max_graczy == 0) ? 0 : round($value->sr_ilosc_graczy / $value->max_graczy*100, 2);
                  // show($value);
